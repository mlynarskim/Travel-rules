   ...
   public func checkOut() {
    ...
    // MARK: - Users Management
        ...
        public func approveLocationRequest(from userId: UUID) {
            userLocationSettings?.visibilitySettings.approvedUsers[userId] = true;           updateNearbyUsers()
        }
        
        public func blockUser(_ userId: UUID) {
            mockUsers.removeAll { $0.id == userId }
            userLocationSettings?.visibilitySettings.approvedUsers.removeValue(forKey: userId);            updateNearbyUsers()
        }
